...
class TownTest {
	...
	void test() 
	{
		...
		for (int i = 0; i < tab.length; ++i) 
		{
			...
			assertEquals(tab[i].isSchool(), false); // On vérifie que chaque ville n'a pas d'école
			assertEquals(tab[0].getName(), 'A'); // On vérifie que la 1ere ville est nommé 'A'
			assertEquals(tab[0].getSerial(), 1); // On vérifie que la 1ere ville a un serial "1"
